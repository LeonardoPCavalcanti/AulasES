...
func TestAlbumEndpoints(t *testing.T) {
	...
	// Helper function to create a request with optional token
	makeRequest := func(method, url, body string, token string) *http.Request {
	...
	getToken := func(username, password string) string {
		credentials := fmt.Sprintf(`{"username": "%s", "password": "%s"}`, username, password)
		req := makeRequest("POST", "/auth", credentials, "")
	...
	// Test cases
	t.Run("GET /albums (no auth)", func(t *testing.T) {
		req := makeRequest("GET", "/albums", "", "")
		resp := httptest.NewRecorder()
		router.ServeHTTP(resp, req)
		assert.Equal(t, http.StatusOK, resp.Code)
	})

	t.Run("GET /albums/:id (no auth)", func(t *testing.T) {
		req := makeRequest("GET", "/albums/1", "", "")
		resp := httptest.NewRecorder()
		router.ServeHTTP(resp, req)
		assert.Equal(t, http.StatusOK, resp.Code)
	})

	t.Run("POST /albums (no auth)", func(t *testing.T) {
		req := makeRequest("POST", "/albums", `{"id": "4", "title": "Test Album", "artist": "Test Artist", "price": 9.99}`, "")
		resp := httptest.NewRecorder()
		router.ServeHTTP(resp, req)
		assert.Equal(t, http.StatusUnauthorized, resp.Code)
	})

	t.Run("PUT /albums/:id (no auth)", func(t *testing.T) {
		req := makeRequest("PUT", "/albums/1", `{"title": "Updated Album"}`, "")
		resp := httptest.NewRecorder()
		router.ServeHTTP(resp, req)
		assert.Equal(t, http.StatusUnauthorized, resp.Code)
	})

	t.Run("DELETE /albums/:id (no auth)", func(t *testing.T) {
		req := makeRequest("DELETE", "/albums/1", "", "")
		resp := httptest.NewRecorder()
		router.ServeHTTP(resp, req)
		assert.Equal(t, http.StatusUnauthorized, resp.Code)
	})

	t.Run("POST /albums (with USER token)", func(t *testing.T) {
		userToken := getToken("testuser", "password") // Assuming you have a testuser
		req := makeRequest("POST", "/albums", `{"id": "4", "title": "Test Album", "artist": "Test Artist", "price": 9.99}`, userToken)
		resp := httptest.NewRecorder()
		router.ServeHTTP(resp, req)
		assert.Equal(t, http.StatusForbidden, resp.Code)
	})

	t.Run("PUT /albums/:id (with USER token)", func(t *testing.T) {
		userToken := getToken("testuser", "password")
		req := makeRequest("PUT", "/albums/1", `{"title": "Updated Album"}`, userToken)
		resp := httptest.NewRecorder()
		router.ServeHTTP(resp, req)
		assert.Equal(t, http.StatusForbidden, resp.Code)
	})

	t.Run("DELETE /albums/:id (with USER token)", func(t *testing.T) {
		userToken := getToken("testuser", "password")
		req := makeRequest("DELETE", "/albums/1", "", userToken)
		resp := httptest.NewRecorder()
		router.ServeHTTP(resp, req)
		assert.Equal(t, http.StatusForbidden, resp.Code)
	})

	t.Run("POST /albums (with ADMIN token)", func(t *testing.T) {
		adminToken := getToken("admin", "password")
		req := makeRequest("POST", "/albums", `{"id": "4", "title": "Test Album", "artist": "Test Artist", "price": 9.99}`, adminToken)
		resp := httptest.NewRecorder()
		router.ServeHTTP(resp, req)
		assert.Equal(t, http.StatusCreated, resp.Code) // Assuming successful POST returns 201
	})

	t.Run("PUT /albums/:id (with ADMIN token)", func(t *testing.T) {
		adminToken := getToken("admin", "password")
		req := makeRequest("PUT", "/albums/4", `{"title": "Updated Album"}`, adminToken)
		resp := httptest.NewRecorder()
		router.ServeHTTP(resp, req)
		assert.Equal(t, http.StatusOK, resp.Code)
	})

	t.Run("DELETE /albums/:id (with ADMIN token)", func(t *testing.T) {
		adminToken := getToken("admin", "password")
		req := makeRequest("DELETE", "/albums/4", "", adminToken)
		resp := httptest.NewRecorder()
		router.ServeHTTP(resp, req)
		assert.Equal(t, http.StatusOK, resp.Code)
	})
