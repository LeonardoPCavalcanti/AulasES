...
func main() {
	...
	adminRoutes.Use(authMiddleware("ADMIN")) // Aplica o middleware com o papel "ADMIN"
	{
	...
	protected.Use(authMiddleware("")) // Não especifica um papel
	{
...
func authenticate(c *gin.Context) {
	...
	for _, u := range users {
		if u.Username == creds.Username && checkPasswordHash(creds.Password, u.Password) {
			token, err := generateToken(creds.Username, "USER") // Passa o papel "USER"
			if err != nil {
