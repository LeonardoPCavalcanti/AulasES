...
var users = []User{
	...
	{Username: "user1", Password: hashPassword("password123")},
}
...
func main() {
	...
	// ... (Criação do diretório de logs, middleware de logging - sem mudanças)
	...
	protected.Use(authMiddleware("")) // Qualquer usuário autenticado
	{
...
func authMiddleware(requiredRole string) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if claims.Role != "ADMIN" && requiredRole == "ADMIN" {
...
func getProtectedData(c *gin.Context) {
	...
	c.IndentedJSON(http.StatusOK, gin.H{"message": fmt.Sprintf("Hello, %s (role: %s)! This is protected data.", username, role)})
}
...
func TestAlbumEndpoints(t *testing.T) {
	...
	router.ServeHTTP(resp, req)
	if resp.Code != http.StatusOK {
		t.Errorf("GET /albums/1 failed, expected %d, got %d", http.StatusOK, resp.Code)
	}
	...
	if resp.Code != http.StatusUnauthorized {
		t.Errorf("POST /albums (without token) failed, expected %d, got %d", http.StatusUnauthorized, resp.Code)
	}

	// Test PUT (should fail without token)
	updatedAlbum := `{"title": "Kind of Blue (Deluxe)", "artist": "Miles Davis", "price": 59.99}`
	req, _ = http.NewRequest("PUT", "/albums/1", strings.NewReader(updatedAlbum))
	req.Header.Set("Content-Type", "application/json")
	resp = httptest.NewRecorder()
	router.ServeHTTP(resp, req)
	if resp.Code != http.StatusUnauthorized {
		t.Errorf("PUT /albums/1 (without token) failed, expected %d, got %d", http.StatusUnauthorized, resp.Code)
	}

	// Test DELETE (should fail without token)
	req, _ = http.NewRequest("DELETE", "/albums/1", nil)
	resp = httptest.NewRecorder()
	router.ServeHTTP(resp, req)
	if resp.Code != http.StatusUnauthorized {
		t.Errorf("DELETE /albums/1 (without token) failed, expected %d, got %d", http.StatusUnauthorized, resp.Code)
	}
}
